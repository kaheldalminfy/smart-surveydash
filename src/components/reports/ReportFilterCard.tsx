        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label className="text-sm">{t('reports.selectQuestionFilter')}</Label>
              <Select value={filterQuestion} onValueChange={onFilterQuestionChange}>
                <SelectTrigger><SelectValue placeholder={t('reports.selectQuestionPlaceholder')} /></SelectTrigger>
                <SelectContent>
                  {allQuestions.filter(q => q.type !== 'text').map((q, i) => (
                    <SelectItem key={q.id} value={q.id}>
                      <span className="flex items-center gap-2">
                        <Badge variant="outline" className="text-xs">
                          {q.type === 'mcq' ? t('reports.choiceLabel') : q.type === 'likert' ? t('reports.likertLabel') : t('reports.ratingLabel')}
                        </Badge>
                        {t('reports.qPrefix')}{i + 1}: {q.text.substring(0, 35)}{q.text.length > 35 ? '...' : ''}
                      </span>
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="space-y-2">
              <Label className="text-sm">{t('reports.selectValues')}</Label>
              <div className={`border rounded-lg p-3 max-h-40 overflow-y-auto ${!filterQuestion ? 'opacity-50 pointer-events-none' : ''}`}>
                {!filterQuestion ? (
                  <p className="text-sm text-muted-foreground text-center">{t('reports.selectQuestionFirst')}</p>
                ) : getFilterOptions().length === 0 ? (
                  <p className="text-sm text-muted-foreground text-center">{t('reports.noOptions')}</p>
                ) : (
                  <div className="flex flex-wrap gap-2">
                    {getFilterOptions().map((opt: string, i: number) => {
                      ...
                      return (
                        <Badge key={i} variant={isSelected ? "default" : "outline"} className="cursor-pointer px-3 py-1" onClick={() => onFilterValueChange(opt)}>
                          {isLikert ? `${opt}/5` : opt}
                        </Badge>
                      );
                    })}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 h-10 px-3 rounded-md border bg-muted/50">
                  ...
                  <span className="text-sm text-muted-foreground">{t('reports.respondent')}</span>
                </div>
              </div>
              <div className="space-y-2">
                <Label className="text-sm font-medium">{t('reports.responseRateLabel')}</Label>
                {(() => {
                  ...
                  if (!manualEnrollment || isNaN(manualNum) || manualNum <= 0) {
                    return (
                      <div className="flex items-center gap-2 h-10 px-3 rounded-md border bg-muted/30">
                        <span className="text-sm text-muted-foreground">{t('reports.enterToCalc')}</span>
                      </div>
                    );
                  }
                  ...
                  return (
                    <div className="space-y-1">
                      <div className="flex items-center gap-2 h-10 px-3 rounded-md border bg-muted/50">
                        ...
                        <span className="text-sm text-muted-foreground">({filteredResponsesCount} / {manualNum})</span>
                      </div>
                      <Progress value={rate} className="h-2" />
                      <p className="text-xs text-muted-foreground">
                        {rate < 50 ? t('reports.lowRate') : rate >= 70 ? t('reports.goodRate') : t('reports.acceptableRate')}
                      </p>
                    </div>
                  );
                })()}
