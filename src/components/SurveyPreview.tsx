...
const SurveyPreview = ({ isOpen, onClose, survey, questions }: SurveyPreviewProps) => {
  ...
  const typeLabels: Record<string, string> = {
    likert: t('designer.likert'),
    text: t('designer.text'),
    mcq: t('designer.mcq'),
    rating: t('designer.rating'),
  };
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={() => window.print()}>
                <Printer className="h-4 w-4 ml-2" />{t('preview.print')}
              </Button>
        ...
        <div className="space-y-6 print:space-y-4">
          <Card className="print:shadow-none print:border-2">
            <CardHeader>
              ...
              {survey.description && <p className="text-muted-foreground text-center mt-2">{survey.description}</p>}
              <div className="flex justify-center gap-2 mt-4">
                <Badge variant="outline">{survey.isAnonymous ? t('preview.anonymous') : t('preview.named')}</Badge>
                <Badge variant="secondary">{questions.length} {t('designer.questions')}</Badge>
              </div>
            </CardHeader>
          </Card>
            ...
            <Card key={question.id} className="print:shadow-none print:border print:break-inside-avoid">
              <CardContent className="pt-6">
                <div className="flex items-start gap-2 mb-4">
                  <Badge variant="outline" className="print:border-2">{index + 1}</Badge>
                  <div className="flex-1">
                    <Label className="text-base font-semibold">
                      {question.text}
                      {question.required && <span className="text-destructive mr-1">*</span>}
                    </Label>
                    <Badge variant="secondary" className="mr-2 mt-2">{typeLabels[question.type] || question.type}</Badge>
                  </div>
                </div>
                ...
                {question.type === "rating" && (
                  <div className="flex gap-1 mr-8">
                    {[1, 2, 3, 4, 5].map((star) => (<Star key={star} className="h-8 w-8 text-muted-foreground" />))}
                  </div>
                )}
                {question.type === "text" && (
                  <Textarea placeholder={t('takeSurvey.answerPlaceholder')} className="mr-8 resize-none print:min-h-32" rows={4} disabled />
                )}
              </CardContent>
            </Card>
          ))}
          ...
          <Card className="print:shadow-none print:border-2">
            <CardContent className="pt-6 text-center">
              <p className="text-muted-foreground">{t('preview.thankYou')}</p>
              <p className="text-sm text-muted-foreground mt-2">{t('preview.helpImprove')}</p>
            </CardContent>
          </Card>
        </div>
      </DialogContent>
